...
<template>
  ...
  >
    ...
    >
      <div class="flex-c flex-1 min-w-0 leading-15" style="display: flex">
        ...
        <div class="flex-c c-p" @click="toHome" v-if="isTopMenu">
          ...
          <p v-if="width >= 1400" class="my-0 mx-2 ml-2 text-lg">{{ brand.name }}</p>
        </div>
      ...
      <div class="flex-c gap-2.5">
        ...
        <ArtIconButton
          ...
          @click="visibleNotice"
        >
          <span v-if="noticeCount" class="notice-badge">{{
    ...
    <!-- 通知 -->
    <ArtNotification v-model:value="showNotice" ref="notice" @count-change="noticeCount = $event" />
  </div>
</template>
