...
<script setup lang="ts">
  ...
  import { qifuTrackAdminUsage } from '@/api/qifu'
  ...
  const primaryMenuEvents: Record<string, string> = {
    '/dashboard': 'admin_menu_workbench',
    '/system': 'admin_menu_system_config',
    '/content': 'admin_menu_content_operations',
    '/maintenance': 'admin_menu_maintenance_tools',
    '/about': 'admin_menu_about_system'
  }

  const trackPrimaryMenu = (item: AppRouteRecord): void => {
    if (props.level !== 0) return
    const event = primaryMenuEvents[item.path || '']
    if (!event) return
    void qifuTrackAdminUsage(event).catch(() => undefined)
  }
