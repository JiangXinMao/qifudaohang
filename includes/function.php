...
function real_ip(){
	...
	if(!defined('QIFU_TRUST_PROXY') || !QIFU_TRUST_PROXY) return $remote;
...
{
    $url = 'https://int.dpool.sina.com.cn/iplookup/iplookup.php?format=json&ip=';
...
function getSetting($k, $force = false){
	global $DB,$CACHE;
	if($force) return $DB->prepared_row('SELECT v FROM web_config WHERE k=? LIMIT 1', array((string)$k));
	$cache = $CACHE->get($k);
	return isset($cache[$k]) ? $cache[$k] : null;
}
function saveSetting($k, $v){
	global $DB;
	$k = (string)$k;
	$v = (string)$v;
	if(!preg_match('/^[a-z0-9_]{1,32}$/i', $k)) return false;
	if(defined('SQLITE')) return $DB->prepared_query('INSERT OR REPLACE INTO web_config (k,v) VALUES (?,?)', array($k,$v));
	return $DB->prepared_query('INSERT INTO web_config (k,v) VALUES (?,?) ON DUPLICATE KEY UPDATE v=VALUES(v)', array($k,$v));
}
...
function writeLog($action, $target, $target_id=0, $detail=''){
	global $DB;
	$ip = real_ip();
	$DB->prepared_query('INSERT INTO web_log (action,target,target_id,detail,ip,addtime) VALUES (?,?,?,?,?,?)', array(mb_substr((string)$action,0,50),mb_substr((string)$target,0,50),intval($target_id),mb_substr((string)$detail,0,255),$ip,time()));
}
?>
